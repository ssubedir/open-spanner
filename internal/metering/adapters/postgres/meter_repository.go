...
func (r *MeterRepository) Save(ctx context.Context, meter domainmeter.Meter) (domainmeter.Meter, error) {
	...
	err = r.store.WithinTransaction(ctx, func(txCtx context.Context) error {
		existing, err := queriesFor(txCtx, r.queries).ListMeters(txCtx, postgresdb.ListMetersParams{
			WorkspaceID: workspaceID,
			ID:          optionalString(meter.ID()),
			Limit:       1,
		})
		if err != nil {
			return err
		...
		err = queriesFor(txCtx, r.queries).SaveMeter(txCtx, postgresdb.SaveMeterParams{
			ID:                 meter.ID(),
			WorkspaceID:        workspaceID,
			Name:               meter.Name(),
		...
		return queriesFor(txCtx, r.queries).IncrementWorkspaceMeters(txCtx, postgresdb.IncrementWorkspaceMetersParams{
			WorkspaceID: workspaceID,
			Delta:       1,
...
func (r *MeterRepository) Delete(ctx context.Context, query domainmeter.Query) error {
	...
	meter := meters[0]
	usageCount, err := queriesFor(ctx, r.queries).CountUsageEventsForMeter(ctx, postgresdb.CountUsageEventsForMeterParams{
		WorkspaceID: workspaceID,
	...
	return r.store.WithinTransaction(ctx, func(txCtx context.Context) error {
		if err := queriesFor(txCtx, r.queries).DeleteMeter(txCtx, postgresdb.DeleteMeterParams{
			WorkspaceID: workspaceID,
			ID:          meter.ID(),
		}); err != nil {
			return err
