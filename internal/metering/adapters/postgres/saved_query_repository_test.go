...
func TestIntegrationSavedQueryRepositoryCRUD(t *testing.T) {
	ctx := context.Background()
	store := newIntegrationStore(t, ctx)

	authRepo := NewAuthRepository(store)
	user := appauth.User{ID: "saved-query-user", Email: "saved-query@example.com", PasswordHash: "hash", CreatedAt: time.Now().UTC()}
