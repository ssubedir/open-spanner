...
func (r *SystemRepository) FindIngestionSafety(ctx context.Context) (appsystem.IngestionSafety, error) {
	...
	err = r.store.QueryRowContext(ctx, `SELECT
		COALESCE((SELECT SUM(accepted) FROM usage_ingestions WHERE workspace_id = $1), 0),
		COALESCE((SELECT SUM(failed) FROM usage_ingestions WHERE workspace_id = $1), 0),
		COALESCE((SELECT ingestion_throttled FROM workspace_stats WHERE workspace_id = $1), 0)`, workspaceID).
