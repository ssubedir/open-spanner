...
func (r *SavedQueryRepository) Save(ctx context.Context, query appsavedquery.SavedQuery) (appsavedquery.SavedQuery, error) {
	...
	err = queriesFor(ctx, r.queries).SaveSavedQuery(ctx, postgresdb.SaveSavedQueryParams{
		ID:          query.ID,
		UserID:      query.UserID,
		Name:        query.Name,
...
func (r *SavedQueryRepository) Find(ctx context.Context, query appsavedquery.FindQuery) ([]appsavedquery.SavedQuery, error) {
	if query.ID != "" {
		row, err := queriesFor(ctx, r.queries).FindSavedQueryByID(ctx, postgresdb.FindSavedQueryByIDParams{
			UserID: query.UserID,
			ID:     query.ID,
		})
...
func (r *SavedQueryRepository) Delete(ctx context.Context, userID string, id string) error {
	rows, err := queriesFor(ctx, r.queries).DeleteSavedQuery(ctx, postgresdb.DeleteSavedQueryParams{UserID: userID, ID: id})
	if err != nil {
		return err
	}
	if rows == 0 {
