...
VALUES (sqlc.arg('workspace_id'), sqlc.arg('idempotency_key'), sqlc.arg('response'), sqlc.arg('subject'), sqlc.arg('meter_name'), sqlc.arg('accepted'), sqlc.arg('evaluation_failed'), sqlc.arg('enforcement'), sqlc.arg('state'), sqlc.arg('created_at'))
...
FROM consumption_decisions
WHERE workspace_id = sqlc.arg('workspace_id')
	AND (sqlc.arg('subject') = '' OR subject = sqlc.arg('subject'))
	AND (sqlc.arg('meter_name') = '' OR meter_name = sqlc.arg('meter_name'))
	AND (sqlc.narg('accepted') IS NULL OR accepted = sqlc.narg('accepted'))
	AND (sqlc.narg('evaluation_failed') IS NULL OR evaluation_failed = sqlc.narg('evaluation_failed'))
	AND (sqlc.arg('enforcement') = '' OR enforcement = sqlc.arg('enforcement'))
	AND (sqlc.arg('state') = '' OR state = sqlc.arg('state'))
	AND (sqlc.narg('cursor_created_at') IS NULL OR created_at < sqlc.narg('cursor_created_at')
