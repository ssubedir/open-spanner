...
func (r *UsageRepository) CountEvents(ctx context.Context) (int, error) {
	var count int
	if err := r.store.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM usage_events`).Scan(&count); err != nil {
		return 0, err
	}
	return count, nil
}
...
`)
	args := []any{}
	if query.HasCursor() {
		sqlQuery.WriteString("HAVING MAX(event_time) < ? OR (MAX(event_time) = ? AND subject > ?)\n")
		cursorTime := formatTime(query.LastEventAt())
		args = append(args, cursorTime, cursorTime, query.Subject())
	}
...
`)
	args := []any{}
	if query.HasCursor() {
		sqlQuery.WriteString(" AND (created_at < ? OR (created_at = ? AND id < ?))\n")
		cursorTime := formatTime(query.CreatedAt())
		args = append(args, cursorTime, cursorTime, query.ID())
	}
...
func (r *UsageRepository) CountPruneRuns(ctx context.Context) (int, error) {
	var count int
	if err := r.store.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM usage_prune_runs`).Scan(&count); err != nil {
		return 0, err
	}
	return count, nil
}
