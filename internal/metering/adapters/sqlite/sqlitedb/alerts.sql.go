...
SET status = 'running', attempts = alert_delivery_jobs.attempts + 1, locked_until = ?1, updated_at = ?2
WHERE id = (
	SELECT candidate.id FROM alert_delivery_jobs candidate
	WHERE candidate.next_attempt_at <= ?2
		AND (candidate.status = 'pending' OR (candidate.status = 'running' AND candidate.locked_until < ?2))
...
UPDATE alert_delivery_jobs
SET status = 'delivered', locked_until = NULL, last_error = '', delivered_at = ?1, updated_at = ?1
WHERE public_id = ?2 AND workspace_id = ?3 AND status = 'running'
`
...
UPDATE alert_delivery_jobs
SET status = 'pending', attempts = 0, next_attempt_at = ?1, locked_until = NULL,
	last_error = '', delivered_at = NULL, updated_at = ?1
WHERE public_id = ?2 AND workspace_id = ?3 AND status = 'dead_letter'
`
...
WHERE public_id = ?5 AND workspace_id = ?6 AND status = 'running'
`
