...
func (r *UsageRepository) queryBucketsWithDynamicSQL(ctx context.Context, query domainusage.Query) ([]domainusage.Bucket, error) {
	...
	rankedSelects = append(rankedSelects,
		goqu.C("quantity"),
		firstRankExpression(groupAliases).As("first_rank"),
		lastRankExpression(groupAliases).As("last_rank"),
	)
...
func firstRankExpression(groupAliases []string) exp.LiteralExpression {
	return goqu.L("ROW_NUMBER() OVER (PARTITION BY " + groupPartitionColumns(groupAliases) + " ORDER BY event_at ASC, id ASC)")
}

func lastRankExpression(groupAliases []string) exp.LiteralExpression {
	return goqu.L("ROW_NUMBER() OVER (PARTITION BY " + groupPartitionColumns(groupAliases) + " ORDER BY event_at DESC, id DESC)")
}
