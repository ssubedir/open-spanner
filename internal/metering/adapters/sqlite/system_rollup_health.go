...
func (r *SystemRepository) ListRollupMeterCoverage(ctx context.Context) ([]appsystem.RollupMeterCoverage, error) {
	...
	rows, err := r.store.QueryContext(ctx, `
		WITH latest AS (
			...
			FROM usage_rollup_runs WHERE workspace_id = ?
		)
		SELECT m.name, m.event_retention_days, r.finalized_through, r.source_events,
			...
			(SELECT COUNT(*) FROM usage_hourly_rollups h
			 WHERE h.workspace_id = m.workspace_id AND h.meter_name = m.name AND (
				...
				h.bucket_start <> strftime('%Y-%m-%dT%H:00:00Z', h.bucket_start) OR
				julianday(h.first_event_time) < julianday(h.bucket_start) OR
				julianday(h.first_event_time) >= julianday(h.bucket_start, '+1 hour') OR
				julianday(h.last_event_time) < julianday(h.bucket_start) OR
				julianday(h.last_event_time) >= julianday(h.bucket_start, '+1 hour')
			 )),
		...
		WHERE m.workspace_id = ? AND m.event_retention_days > 0
		ORDER BY m.name`, workspaceID, workspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
