...
| `metadata_schema` | Dimension contract for accepted event metadata. |
...
Future versions may add richer meter types and dimension workflows. For now, the stable contract is the meter definition plus metadata schema, and that is the surface your services should depend on.
