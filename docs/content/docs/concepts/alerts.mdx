...
Alert evaluation and webhook delivery run outside the API process. Accepted
usage first enters the transactional usage outbox; the usage worker durably
queues matching alert work, and the alert worker evaluates rules and delivers
their events.
