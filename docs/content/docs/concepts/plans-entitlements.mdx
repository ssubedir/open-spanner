...
Plan management is a control-plane workflow. Backend services should not create
plans from SDK code during normal application traffic.
...
See [Enforce Entitlements](/docs/tutorials/check-entitlements) for the recommended
atomic-consumption flow and links to language-specific SDK guides.
...
Quota counters can be checked against source usage and consumption decisions.
See [Quota Reconciliation](/docs/configuration/reconciliation) for read-only
scans, scheduled monitoring, and guarded repair.
...
Progress and status endpoints are part of the SDK surface. Event and period
history are available from the dashboard and full REST API. Creating plans and
assigning subjects remains a control-plane workflow.
