...
An accepted request returns `201`. A hard-limit rejection returns `429` and includes the current, projected, remaining, reset, enforcement, and failure-policy fields. Retrying an accepted idempotency key returns the original event without consuming quota twice.
