...
Use Docker Run when you want the smallest local instance. Use Docker Compose when you want the API, export worker, alert worker, entitlement worker, Postgres, and shared export storage started together.
...
Use `latest` for a quick trial. For production, pin a release tag such as `ssubedir/open-spanner:0.1.12`.

Queued export jobs, alerts, and entitlement state updates use worker processes. Start workers against the same Docker volume when you want dashboard export jobs to produce downloadable files, alert rules to evaluate, or quota state history to refresh outside the API process:
...
docker rm -f open-spanner-entitlement-worker
...
This starts the Open Spanner API, export worker, alert worker, entitlement worker, and Postgres together. Postgres is private to the Compose network and is not published on host port `5432`.
