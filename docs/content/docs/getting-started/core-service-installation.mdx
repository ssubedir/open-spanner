...
Open Spanner can run as a single container with SQLite or as an app, export worker, and Postgres stack with Docker Compose.

Use Docker Run when you want the smallest local instance. Use Docker Compose when you want the API, export worker, Postgres, and shared export storage started together.
...
Queued export jobs need a worker process. Start a worker against the same Docker volume when you want dashboard export jobs to produce downloadable files:
...
This starts the Open Spanner API, export worker, and Postgres together. Postgres is private to the Compose network and is not published on host port `5432`.
...
Run the export worker in another terminal when you want queued export jobs to produce downloadable files:
