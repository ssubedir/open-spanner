...
Open Spanner runs a Next.js dashboard in front of a private Go API. The dashboard owns the public HTTP endpoint and proxies `/v1` requests to the API at runtime. The API stores metering data in SQLite or Postgres, exposes REST for dashboard and SDK control-plane operations, and exposes gRPC for trusted backend usage ingestion.
