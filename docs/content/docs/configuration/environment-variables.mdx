...
| `OPEN_SPANNER_USAGE_DB_MAX_OPEN_CONNS` / `OPEN_SPANNER_USAGE_DB_MAX_IDLE_CONNS` | `5` / `2` | Pool budget passed to the usage-outbox-worker container. |
...
| `OPEN_SPANNER_USAGE_WORKER_*` | service defaults | Passed through to the usage-outbox worker container. |
