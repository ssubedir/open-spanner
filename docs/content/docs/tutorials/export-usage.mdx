...
CSV exports run from the API request today. Keep export windows focused and use filters when possible, especially when a meter has a high event volume. Direct exports default to 100 rows and cannot request more than 1000 rows. Background export processing is planned, but the current file-producing endpoints return the CSV directly.
